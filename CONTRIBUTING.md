...
3. Build with `./scripts/build` and commit only source and metadata. `dist/` is
   ignored.
...
`go mod tidy`, and rebuild all packages twice before merging. ABI changes
require a new catalog ABI and must not rewrite existing releases.
